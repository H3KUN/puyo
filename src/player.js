class Player{
    ...
    static falling(isDownPressed){
        ...
        if(!isBlocked){
            ...
            if(Math.floor(this.puyoStatus.top/Config.puyoImgHeight)!=y){
                ...
                y+=1;
                this.puyoStatus.y=y;
                if(y+1>=Config.stageRows||Stage.board[y+1][x]||(y+dy+1>=0&&(y+dy+1>=Config.stageRows||Stage.board[y+dy+1][x+dx])))isBlocked=true;
                if(!isBlocked){
                    this.groundFrame=0;
                    return;
                }else{
        ...
        if(this.groundFrame==0){
            ...
            this.groundFrame++;
            if(this.groundFrame>Config.playerGroundFrame)return true;
        }
    }
    static playing(frame){
        ...
        if(this.keyStatus.right||this.keyStatus.left){
            ...
            let canMove = true;
            if(y<0||x+cx<0||x+cx>=Config.stageCols||Stage.board[y][x+cx])if(y>=0)canMove=false;
            if(my<0||mx+cx<0||mx+cx>=Config.stageCols||Stage.board[my][mx+cx])if(my>=0)canMove=false;
            if(this.groundFrame===0){
                if(y+1<0||x+cx<0||x+cx>=Config.stageCols||Stage.board[my+1][mx+cx])if(y+1>=0)canMove=false;
                if(my+1<0||x+cx<0||x+cx>=Config.stageCols||Stage.board[my+1][mx+cx])if(my+1>=0)canMove=false;
            }
            if(canMove){
            ...
            let cy = 0;
            if(rotation===90){
                if(y+1<0||x-1<0||x-1>=Config.stageCols||Stage.board[y+1][x-1])if(y+1>=0)cx=1;
                if(cx===1)if(y+1<0||x+1<0||y+1>=Config.stageRows||x+1>=Config.stageCols||Stage.board[y+1][x+1])if(y+1>=0)canRotate=false;
            }else if(rotation===180){
                if(y+2<0||y+2>=Config.stageRows||Stage.board[y+2][x])if(y+2>=0)cy=-1;
                if(y+2<0||y+2>=Config.stageRows||x-1<0||Stage.board[y+2][x-1])if(y+2>=0)cy=-1;
            }else if(rotation===270){
                if(y+1<0||x+1<0||x+1>=Config.stageCols||Stage.board[y+1][x+1])if(y+1>=0)cx=-1;
                if(cx===-1)if(y+1<0||x-1<0||x-1>=Config.stageCols||Stage.board[y+1][x-1])if(y+1>=0)canRotate=false;
            }
            if(canRotate){
                if(cy===-1){
                    if(this.groundFrame>0){
    ...
    static batankyu(){
        if(this.keyStatus.up)location.reload()
    }
}